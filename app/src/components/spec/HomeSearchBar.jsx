...
import { setDaily, setIsAuthenticated, setDailyScore, setGameArray } from '../../context/UserSlice';
...
const HomeSearchBar = ({data, gameItems, answer, setScore, setAnimationTrigger, animationTrigger}) => 
{
    ...
    const handleChange = async (selectedOption) =>
    {
      if (selectedOption && selectedOption.value)
      {
        ...
        let updatedGamesItems = [ choice, ...gameItems];
        dispatch(setGameArray({ gameArray : [choice, ...gameItems]}))
        ...
        if (choice.ID === answer.ID)
        {
            ...
            {
                score = getScore({ tries : updatedGamesItems.length , streak : user.user.streak });
            } else {
                score = getScore({ tries : updatedGamesItems.length, streak : 0 });
            }
            setScore(score);
    ...
    if (user.didItToday)
    {
      return (
        <SearchBarContainer>
          <StyledSelect components={{ Option : CustomOption }} onChange={handleChange} value={selectedOption} isDisabled={user.didItToday} isClearable={true} styles={customStyles} options={options} placeholder={'Revenez demain pour choisir un guess'}/>
        </SearchBarContainer>
      )
    } else {
      return (
        <SearchBarContainer>
            <StyledSelect components={{ Option : CustomOption }} onChange={handleChange} value={selectedOption} isDisabled={user.didItToday} isClearable={true} styles={customStyles} options={options} placeholder={'Choisissez votre guess'}/>
        </SearchBarContainer>
      )
    }    
}
